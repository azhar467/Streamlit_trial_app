...
def preview_changes(project_id):
    ...
    try:
        ...
        # If not:                   diff against source (shows what WOULD be committed to a new branch)
        current_ref = _feature_branch if branch_existed else ms.SOURCE_BRANCH
        ...
        return jsonify({
            ...
            'source_branch': ms.SOURCE_BRANCH,
        })
            ...
            try:
                ...
                try:
                    ...
                    if branch_existed:
                        ...
                        # Branch does not exist yet — read files from SOURCE_BRANCH to
                        # build the proposed change set. Branch creation is deferred
                        # until we confirm there are actual changes to make.
                        active_tasks[task_id]['logs'].append({
                            'level': 'INFO',
                            'message': f'🔍 Branch "{_feature_branch}" not found — scanning {ms.SOURCE_BRANCH} to check for required changes…',
                            'timestamp': datetime.now().isoformat()
                        })
                    ...
                    if not actions:
                        if branch_existed:
                            ...
                            msg = f'✅ No changes required — {ms.SOURCE_BRANCH} is already fully migrated. No branch created.'
                    ...
                    # Safety-net: branch doesn't exist yet but check_files_already_match
                    # shows the changes are somehow already there (race condition / external commit).
                    # No branch was created by us → use check_done, not committed.
                    ...
                    if not branch_existed:
                        active_tasks[task_id]['logs'].append({
                            'level': 'INFO',
                            'message': f'🌿 {len(actions)} change(s) confirmed — creating branch "{_feature_branch}" from {ms.SOURCE_BRANCH}…',
                            'timestamp': datetime.now().isoformat()
                        })
