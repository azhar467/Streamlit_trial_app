    ...
    try:
        ...
        def deploy_thread():
            ...
            try:
                ...
                # Get feature branch HEAD commit
                branch_info = ms.api_call(
                    f"projects/{project_id}/repository/branches/{urllib.parse.quote(ms.FEATURE_BRANCH, safe='')}"
                )
                feature_head = None
                ...
                if not feature_head:
                    _log('ERROR', f'Could not resolve HEAD of branch {ms.FEATURE_BRANCH}')
                    active_tasks[task_id]['status'] = 'failed'
                    return
                ...
                for tag_name in tags:
                    ...
                    if tag_commit_id and tag_commit_id == feature_head:
                        ...
                        _log('INFO', f'Creating tag "{tag_name}" on {ms.FEATURE_BRANCH} ({feature_head[:8]})...')
                        t_res = ms.api_call(
                            f"projects/{project_id}/repository/tags", 'POST',
                            {'tag_name': tag_name, 'ref': ms.FEATURE_BRANCH}
                        )
...
def reload_config():
    ...
    try:
        ...
        print(f"[INFO] Config reloaded — {len(PROJECT_NAMES)} projects, token_set={bool(ms.TOKEN)}")
        return jsonify({'success': True, 'projects': len(PROJECT_NAMES), 'token_set': bool(ms.TOKEN)})
    except Exception as e:
