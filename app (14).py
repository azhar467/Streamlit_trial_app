...
def _load_extended_env_config():
    ...
    extra = {
        ...
        'presto_project_ids': [],
        'release_management_usernames': [],
        'code_review_usernames': [],
    }
    env_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), '.env')
    if not os.path.exists(env_file):
        return extra
    try:
        with open(env_file, encoding='utf-8-sig', errors='replace') as f:
            for line in f:
                ...
                elif k == 'RELEASE_MANAGEMENT_USERNAMES':
                    extra['release_management_usernames'] = [x.strip() for x in v.split(',') if x.strip()]
                    print(f"[ENV] RELEASE_MANAGEMENT_USERNAMES loaded: {extra['release_management_usernames']}")
                elif k == 'CODE_REVIEW_USERNAMES':
                    extra['code_review_usernames'] = [x.strip() for x in v.split(',') if x.strip()]
                    print(f"[ENV] CODE_REVIEW_USERNAMES loaded: {extra['code_review_usernames']}")
    except Exception as ex:
...
env_config['presto_project_ids']             = _ext_config['presto_project_ids']
env_config['release_management_usernames']   = _ext_config['release_management_usernames']
env_config['code_review_usernames']          = _ext_config['code_review_usernames']
...
def _transform_jfrog_publish_tags(content):
    """
    In the jfrog-publish-tags job, replace the 'only:' block with a 'rules:' block
    that triggers only on commit tags matching the release pattern.

    Uses a line-by-line approach so it handles any YAML indentation reliably.
    ...
    while i < len(lines):
        ...
        # Detect the jfrog-publish-tags job header
        if not (stripped == 'jfrog-publish-tags:' or
                stripped.startswith('jfrog-publish-tags:')):
            result.append(line)
            i += 1
            continue
        ...
        # Scan job body looking for 'only:'
        job_body = []
        only_start_idx = None
        only_key_indent = None

        while i < len(lines):
            l = lines[i]
            # Non-blank line at same/lesser indent as job = next top-level key
            if l.strip() and not l[0].isspace():
                break
            lstripped = l.lstrip()
            curr_indent = (len(l) - len(lstripped)) if l.strip() else 9999
            if l.strip() and curr_indent <= job_indent:
                break

            if lstripped.startswith('only:') and only_start_idx is None:
                only_start_idx = len(job_body)
                only_key_indent = curr_indent

            job_body.append(l)
            i += 1

        # Replace only: block with rules: if found
        if only_start_idx is not None:
            # Find end of only list - stop when indent drops back to key level
            only_end_idx = only_start_idx + 1
            while only_end_idx < len(job_body):
                candidate = job_body[only_end_idx]
                if candidate.strip() == '':
                    only_end_idx += 1
                    continue
                if (len(candidate) - len(candidate.lstrip())) <= only_key_indent:
                    break
                only_end_idx += 1

            indent_str = ' ' * only_key_indent
            list_indent = ' ' * (only_key_indent + 2)
            rules_lines = [
                f'{indent_str}rules:',
                f'{list_indent}- if: $CI_COMMIT_TAG =~ "/.*-release$/"',
            ]
            job_body = (job_body[:only_start_idx] +
                        rules_lines +
                        job_body[only_end_idx:])

        result.extend(job_body)
...
def _resolve_source_branch(project_id):
    ...
    for candidate in ('develop', 'developer', 'master', 'main'):
...
def preview_changes(project_id):
    ...
    try:
        ...
        # If feature branch exists: diff against it (shows what WILL change on that branch)
        # If not:                   diff against this project's actual default branch.
        #
        # IMPORTANT: use _resolve_source_branch() here, NOT the module-level ms.SOURCE_BRANCH.
        # ms.SOURCE_BRANCH is a global constant (usually 'develop') that may not exist on every
        # project — projects that use 'master' or 'main' as their default would return a 404
        # for every file read, causing actions=[] and silently dropping the project from preview.
        _source_branch = _resolve_source_branch(project_id) if not branch_existed else None
        current_ref = _feature_branch if branch_existed else _source_branch
        ...
        if '2' in choices:  # CI
            try:
                ...
                if isinstance(res, dict) and "content" in res:
                    ...
                    upd = _transform_jfrog_publish_tags(upd)
                    if orig != upd:
                        diff = generate_diff(orig, upd)
        ...
        return jsonify({
            ...
            'branch_used': current_ref,              # which ref the diff was read from
            'branch_existed': branch_existed,        # True = feature branch already exists
            'source_branch': _source_branch or current_ref,  # resolved per-project default
        })
...
def commit_project(project_id):
    ...
    try:
        ...
        choices = data.get('choices', [])
        branch_num = data.get('branch_num', '12938')
        # NOTE: commit message is intentionally left empty here so the background
        # thread can choose the correct contextual default AFTER it has checked
        # whether the feature branch already exists.  See _message logic below.
        custom_commit_message = (data.get('commit_message') or '').strip()
        ...
        def commit_thread():
            ...
            try:
                ...
                try:
                    ...
                    # ── Branch check ────────────────────────────────────────────────
                    # NOTE: We intentionally do NOT create the branch yet.
                    # Branch creation is deferred until after the file scan confirms
                    # there are actual changes to commit (Scenarios 1, 3, 4, 5).
                    br_check = ms.api_call(f"projects/{project_id}/repository/branches/{urllib.parse.quote(_feature_branch, safe='')}")
                    branch_existed = isinstance(br_check, dict) and "name" in br_check

                    # ── Dynamic default commit message ───────────────────────────────
                    # • New branch  → "fix: java 17 migration"
                    # • Existing branch (platform bump / re-run) → "fix: platform upgrade to X.Y.Z"
                    # Only applies when the caller sent no explicit message (or the
                    # frontend sent the generic auto-generated default).
                    _GENERIC_DEFAULTS = {
                        'fix: java17-migration',
                        'fix: java 17 migration',
                        f'fix: TASK-{_jira_id}-java17-migration',
                        f'fix: TASK-{_jira_id}-{data.get("branch_suffix","java17-migration")}',
                    }
                    if not _message or _message in _GENERIC_DEFAULTS:
                        if branch_existed:
                            _message = f"fix: platform upgrade to {ms.NEW_DEFAULT_PLATFORM}"
                        else:
                            _message = "fix: java 17 migration"
                    # ────────────────────────────────────────────────────────────────

                    if branch_existed:
                        # Branch already exists — read files directly from it so we
                        # detect only what still needs changing on THIS branch.
                        active_tasks[task_id]['logs'].append({
                            'level': 'INFO',
                        ...
                        _source_branch = None   # not needed; branch exists
                    else:
                        # Branch does not exist yet — resolve THIS project's actual default branch
                        # (develop / master / main) rather than using the module-level ms.SOURCE_BRANCH
                        # constant which may not exist on every project.  Reading from a non-existent
                        # branch returns 404 for every file, making actions=[] and producing a false
                        # idempotent result.
                        _source_branch = _resolve_source_branch(project_id)
                        active_tasks[task_id]['logs'].append({
                            'level': 'INFO',
                            'message': f'🔍 Branch "{_feature_branch}" not found — scanning {_source_branch} to check for required changes…',
                            'timestamp': datetime.now().isoformat()
                        })
                        current_ref = _source_branch
                    ...
                    if '3' in choices:  # EB Config
                        ...
                        if isinstance(res, dict) and "content" in res:
                            ...
                            upd = re.sub(r"(default_platform:\s*).*$", f"default_platform: {ms.NEW_DEFAULT_PLATFORM}", orig, flags=re.MULTILINE)
                            if orig != upd:
                                # Detect platform-version-only re-commit and log clearly
                                _old_plat = re.search(r'default_platform:\s*(.+)', orig)
                                _new_plat = ms.NEW_DEFAULT_PLATFORM
                                if _old_plat and branch_existed:
                                    _old_val = _old_plat.group(1).strip()
                                    active_tasks[task_id]['logs'].append({
                                        'level': 'INFO',
                                        'message': (
                                            f'  .elasticbeanstalk/config.yml → platform version changed: '
                                            f'"{_old_val}" → "{_new_plat}" ✏️'
                                        ),
                                        'timestamp': datetime.now().isoformat()
                                    })
                                else:
                                ...
                                active_tasks[task_id]['logs'].append({'level': 'INFO', 'message': f'  .elasticbeanstalk/config.yml → platform "{ms.NEW_DEFAULT_PLATFORM}" already set ✓', 'timestamp': datetime.now().isoformat()})
                        elif isinstance(res, dict) and res.get('error'):
                    ...
                    if not actions:
                        if branch_existed:
                            # S4: branch exists, all files already match — nothing new to commit.
                            # Keep workflow_stage='committed' because a real commit WAS made earlier
                            # (that's why the branch exists). We just have nothing to add now.
                            msg = f'✅ No changes required — branch "{_feature_branch}" already has all migration changes. Nothing to commit.'
                            _no_change_stage = 'committed'
                        else:
                            # S1: no branch, source branch is already fully migrated.
                            # No branch was ever created — use 'check_done' so the UI shows
                            # "Already Migrated" and the MR button stays correctly disabled.
                            msg = f'✅ No changes required — {_source_branch} is already fully migrated. No branch created.'
                            _no_change_stage = 'check_done'
                        active_tasks[task_id]['logs'].append({
                            'level': 'INFO',
                            'message': msg,
                            'timestamp': datetime.now().isoformat()
                        ...
                        update_pipeline_status(project_id, 'success', workflow_stage=_no_change_stage,
                                               path_with_namespace=p_path)
                        add_project_to_history(project_id, p_name, 'commit', 'already_done')
                        return

                    # Safety-net: branch didn't exist at the start of this thread, but
                    # check_files_already_match against a **non-existent** branch would
                    # return all-match because every API read returns 404/error.  We must
                    # re-verify the branch actually exists NOW before running the check,
                    # otherwise we produce a false-idempotent result and silently drop
                    # projects that genuinely need a new commit.
                    if not branch_existed:
                        _recheck = ms.api_call(
                            f"projects/{project_id}/repository/branches/"
                            f"{urllib.parse.quote(_feature_branch, safe='')}"
                        )
                        _branch_now_exists = isinstance(_recheck, dict) and "name" in _recheck
                        if _branch_now_exists:
                            # Race condition: someone else created the branch; check it properly
                            all_match, _ = ms.check_files_already_match(project_id, actions, _feature_branch)
                            if all_match:
                                active_tasks[task_id]['logs'].append({
                                    'level': 'INFO',
                                    'message': f'✅ All changes are already present on {_feature_branch} — no new commit needed. No branch created by this run.',
                                    'timestamp': datetime.now().isoformat()
                                })
                                active_tasks[task_id]['status'] = 'idempotent'
                                update_pipeline_status(project_id, 'success', workflow_stage='check_done',
                                                       path_with_namespace=p_path)
                                add_project_to_history(project_id, p_name, 'commit', 'already_committed')
                                return
                        # Branch still doesn't exist → proceed to create it below

                    # ── Changes confirmed — now create the branch if it doesn't exist ──
                    # Branch creation is placed HERE (after all checks) so:
                    #   • No empty branches are ever created (Scenario 1 ✅)
                    #   • Safety-net idempotent case also skips branch creation (Scenario 3 ✅)
                    #   • Existing branch is reused as-is (Scenario 5 ✅)
                    if not branch_existed:
                        active_tasks[task_id]['logs'].append({
                            'level': 'INFO',
                            'message': f'🌿 {len(actions)} change(s) confirmed — creating branch "{_feature_branch}" from {_source_branch}…',
                            'timestamp': datetime.now().isoformat()
                        })
                        with _ms_globals_lock:
                            _orig_source_branch = ms.SOURCE_BRANCH   # save for restore
                            ms.FEATURE_BRANCH   = _feature_branch
                            ms.JIRA_ID          = _jira_id
                            ms.SOURCE_BRANCH    = _source_branch   # point to THIS project's real default
                            try:
                                ms.create_feature_branch(project_id, p_name)
                            finally:
                                ms.SOURCE_BRANCH = _orig_source_branch   # always restore
                    ...
                    # If branch existed and all actions are EB config-only (platform bump),
                    # use a more descriptive re-commit message automatically.
                    _is_platform_only_recommit = (
                        branch_existed and
                        len(actions) == 1 and
                        actions[0]['file_path'] == '.elasticbeanstalk/config.yml'
                    )
                    if _is_platform_only_recommit and _message == 'fix: java17-migration':
                        _message = f'fix: update platform to {ms.NEW_DEFAULT_PLATFORM}'
                        active_tasks[task_id]['logs'].append({
                            'level': 'INFO',
                            'message': f'📝 Re-commit detected (platform version change only) — commit message: "{_message}"',
                            'timestamp': datetime.now().isoformat()
                        })
                    ...
                    # Commit changes
                    _commit_verb = 'Re-committing' if branch_existed else 'Committing'
                    active_tasks[task_id]['logs'].append({
                        'level': 'INFO',
                        'message': f'{_commit_verb} {len(actions)} file(s) to {_feature_branch}...',
                        'timestamp': datetime.now().isoformat()
                    })
...
def create_mr(project_id):
    ...
    Accepted body fields:
      branch_num            str   - Jira ticket number (default '12938')
      branch_prefix         str   - branch prefix (default 'task-')
      branch_suffix         str   - branch suffix (default 'java17-migration')
      mr_type               str   - 'feature_to_develop' (default) or 'develop_to_master'
      assignees             list  - GitLab usernames to assign (overrides .env defaults)
      reviewers             list  - GitLab usernames as reviewers (overrides .env defaults)
      release_approvers     list  - usernames from Release Management group
      code_review_approvers list  - usernames from Code Review group
      use_queue             bool  - reserved; per-project MR tasks are already lightweight
    """
    try:
        data = request.json or {}
        branch_num    = str(data.get('branch_num', '12938'))
        branch_prefix = data.get('branch_prefix', 'task-')
        branch_suffix = data.get('branch_suffix', 'java17-migration')
        mr_type           = data.get('mr_type', 'feature_to_develop')
        # When True the backend resolves approvers from this project's own GitLab
        # approval_rules at submission time instead of using the shared UI selection.
        use_project_rules = bool(data.get('use_project_rules', False))

        # Merge all reviewer sources: explicit reviewers + release approvers + code review approvers
        _req_reviewers         = [str(u) for u in (data.get('reviewers') or []) if u]
        _release_approvers     = [str(u) for u in (data.get('release_approvers') or []) if u]
        _code_review_approvers = [str(u) for u in (data.get('code_review_approvers') or []) if u]
        # Deduplicate while preserving order
        _all_reviewers = list(dict.fromkeys(_req_reviewers + _release_approvers + _code_review_approvers))

        _req_assignees = [str(u) for u in (data.get('assignees') or []) if u]

        # Compute branch names locally — never mutate ms globals in the request handler
        _feature_branch = _build_feature_branch(branch_num, branch_prefix, branch_suffix)
        _mr_title       = f"TASK-{branch_num}: java migration"
        _jira_id        = branch_num

        task_id = f"{project_id}_mr_{int(time.time() * 1000)}"
        # develop→master MRs start as 'queued'; feature→develop start as 'running'
        _initial_status = 'queued' if mr_type == 'develop_to_master' else 'running'
        active_tasks[task_id] = {
        ...
        # Queue registration for develop→master — enforces serial execution
        _dtm_pid_str   = f"dtm_{project_id}" if mr_type == 'develop_to_master' else None
        _dtm_cancel_ev = _queue_register(_dtm_pid_str) if _dtm_pid_str else None
        ...
        def mr_thread():
            ...
            try:
                ...
                active_tasks[task_id]['logs'].append({
                    'level':     'INFO',
                    'message':   f'Creating MR ({mr_type}) for {p_name}...',
                    'timestamp': datetime.now().isoformat()
                })

                # ── Queue gating for develop→master ────────────────────────
                if _dtm_pid_str and _dtm_cancel_ev:
                    update_pipeline_status(project_id, 'queued')
                    ok = _queue_acquire(_dtm_pid_str)
                    if not ok:
                        active_tasks[task_id]['logs'].append({
                            'level': 'WARN',
                            'message': 'Develop→Master MR cancelled while queued',
                            'timestamp': datetime.now().isoformat()
                        })
                        active_tasks[task_id]['status'] = 'cancelled'
                        update_pipeline_status(project_id, 'cancelled')
                ...
                # ── Effective reviewers / assignees ─────────────────────
                # Priority:
                #   1. use_project_rules=True  → query THIS project's GitLab
                #      approval_rules and use those eligible_approvers
                #   2. Explicit UI selection    → use what the caller sent
                #   3. .env fallback            → REVIEWER/RM/CR usernames
                if use_project_rules:
                    # Auto-resolve from this project's own approval_rules
                    effective_reviewers = []
                    try:
                        _proj_rules = ms.api_call(f"projects/{project_id}/approval_rules")
                        if isinstance(_proj_rules, list) and _proj_rules:
                            _seen_u = set()
                            for _rule in _proj_rules:
                                for _eu in (_rule.get('eligible_approvers') or []):
                                    _uname = _eu.get('username', '')
                                    if _uname and _uname not in _seen_u:
                                        effective_reviewers.append(_uname)
                                        _seen_u.add(_uname)
                            if effective_reviewers:
                                active_tasks[task_id]['logs'].append({
                                    'level': 'INFO',
                                    'message': (
                                        f'🔍 Auto-resolved {len(effective_reviewers)} approver(s) from '
                                        f'project approval rules: {", ".join(effective_reviewers)}'
                                    ),
                                    'timestamp': datetime.now().isoformat()
                                })
                    except Exception as _ar_err:
                        print(f"[MR] approval_rules auto-fetch failed for {project_id}: {_ar_err}")
                else:
                    # Manual mode: use the list the caller sent
                    effective_reviewers = list(_all_reviewers)

                # .env fallback for both modes when nothing was resolved above
                if not effective_reviewers:
                    if mr_type == 'develop_to_master':
                        _rm = env_config.get('release_management_usernames', [])
                        _cr = env_config.get('code_review_usernames', [])
                        effective_reviewers = list(dict.fromkeys(_rm + _cr))
                    if not effective_reviewers:
                        effective_reviewers = list(ms.REVIEWER_USERNAMES or [])
                    if effective_reviewers:
                        _fb_src = 'project approval rules empty' if use_project_rules else 'no reviewers selected'
                        active_tasks[task_id]['logs'].append({
                            'level': 'INFO',
                            'message': (
                                f'ℹ️ {_fb_src} — applying .env defaults: '
                                f'{", ".join(effective_reviewers)}'
                            ),
                            'timestamp': datetime.now().isoformat()
                        })

                effective_assignees = list(_req_assignees) if _req_assignees else list(ms.ASSIGNEE_USERNAMES or [])

                # ── develop→master: skip MR if no changes ─────────────────
                if mr_type == 'develop_to_master':
                    _dev_branch_chk = _resolve_source_branch(project_id)
                    _master_chk     = None
                    for _mc in ('master', 'main'):
                        if _mc == _dev_branch_chk:
                            continue
                        _br = ms.api_call(
                            f"projects/{project_id}/repository/branches/{urllib.parse.quote(_mc, safe='')}"
                        )
                        if isinstance(_br, dict) and _br.get('name') and not _br.get('error'):
                            _master_chk = _mc
                            break
                    ...
                    if _master_chk:
                        _cmp = ms.api_call(
                            ...
                            f"?from={urllib.parse.quote(_master_chk, safe='')}"
                            f"&to={urllib.parse.quote(_dev_branch_chk, safe='')}"
                            f"&straight=false"
                        )
                        if isinstance(_cmp, dict) and not _cmp.get('error'):
                            _ahead = _cmp.get('commits', []) or []
                            if not _ahead:
                                active_tasks[task_id]['logs'].append({
                                    'level': 'INFO',
                                    'message': (
                                        f'✅ No changes detected — {_dev_branch_chk} is already '
                                        f'up-to-date with {_master_chk}. MR not needed.'
                                    ),
                                    'timestamp': datetime.now().isoformat()
                                })
                                active_tasks[task_id]['status'] = 'no_changes'
                                add_project_to_history(
                                    project_id, p_name, 'mr', 'no_changes',
                                    {'reason': f'{_dev_branch_chk} already merged into {_master_chk}'}
                                )
                                _mr_succeeded = True
                                return

                # ── Server-side guard: feature branch must exist for Feature→Develop MR ──
                if mr_type == 'feature_to_develop':
                    _br_chk = ms.api_call(
                        f"projects/{project_id}/repository/branches/{urllib.parse.quote(_feature_branch, safe='')}"
                    )
                    _branch_ok = isinstance(_br_chk, dict) and _br_chk.get('name') and not _br_chk.get('error')
                    if not _branch_ok:
                        active_tasks[task_id]['logs'].append({
                            'level': 'ERROR',
                            'message': (
                                f'❌ Cannot raise MR — feature branch "{_feature_branch}" does not exist in this project. '
                                f'Run the commit step first to create the branch and push changes.'
                            ),
                            'timestamp': datetime.now().isoformat()
                        })
                        active_tasks[task_id]['status'] = 'failed'
                        update_pipeline_status(project_id, 'failed')
                        return

                # Hold the globals lock so concurrent requests don't overwrite each other's
                # FEATURE_BRANCH / REVIEWER_USERNAMES / ASSIGNEE_USERNAMES mid-execution.
                with _ms_globals_lock:
                    ms.FEATURE_BRANCH     = _feature_branch
                    ms.JIRA_ID            = _jira_id
                    ms.MR_TITLE           = _mr_title
                    ms.REVIEWER_USERNAMES = effective_reviewers
                    ms.ASSIGNEE_USERNAMES = effective_assignees

                    if mr_type == 'develop_to_master':
                        # source_branch = develop (or main); target = master/main
                        _orig_source  = ms.SOURCE_BRANCH
                        _orig_feature = ms.FEATURE_BRANCH
                        _dev_branch   = _resolve_source_branch(project_id)
                        ms.FEATURE_BRANCH = _dev_branch
                        ms.SOURCE_BRANCH  = 'master'
                        try:
                            result = ms.create_mr_for_project(project_id, p_name, {'snapshots': []})
                        finally:
                            ms.SOURCE_BRANCH  = _orig_source
                            ms.FEATURE_BRANCH = _orig_feature
                    else:
                        # Default: feature branch → develop
                        result = ms.create_mr_for_project(project_id, p_name, {'snapshots': []})

                if result.get('success'):
                    active_tasks[task_id]['logs'].append({
                        'level':     'SUCCESS',
                        'message':   f'✅ MR created successfully ({mr_type})',
                        'timestamp': datetime.now().isoformat()
                    })
                    active_tasks[task_id]['status'] = 'success'
                    ...
                    active_tasks[task_id]['logs'].append({
                        'level':     'ERROR',
                        'message':   f'❌ MR creation failed: {result.get("error", "unknown")}',
                        'timestamp': datetime.now().isoformat()
                    })
                    active_tasks[task_id]['status'] = 'failed'
                ...
                if _acquired and _dtm_pid_str:
                    _queue_release(_dtm_pid_str, succeeded=_mr_succeeded)
                _cleanup_active_tasks()
...
@app.route('/api/projects/<int:project_id>/develop-master-compare', methods=['GET'])
def develop_master_compare(project_id):
    """Compare develop branch against master/main to determine if there are changes.

    Returns:
        has_changes  bool   – True when develop has commits ahead of master
        commits      list   – commits in develop not yet in master (max 50)
        diff_files   list   – files changed (from GitLab compare API)
        ahead_by     int    – number of commits develop is ahead of master
        develop      str    – name of the source branch used (develop/main/master)
        target       str    – name of the target branch (master/main)
    """
    try:
        # Resolve develop (source) branch
        develop_branch = _resolve_source_branch(project_id)

        # Resolve master/main (target) branch
        master_branch = None
        for candidate in ('master', 'main'):
            if candidate == develop_branch:
                continue
            resp = ms.api_call(
                f"projects/{project_id}/repository/branches/{urllib.parse.quote(candidate, safe='')}"
            )
            if isinstance(resp, dict) and resp.get('name') and not resp.get('error'):
        ...
        if not master_branch:
            return jsonify({
                'success': False,
                'error': 'Could not find master/main branch',
                'has_changes': False,
            }), 404
        ...
        if develop_branch == master_branch:
            return jsonify({
                ...
                'detail': 'Develop and master are the same branch',
            })

        # Use GitLab compare API: from=master, to=develop → shows what develop has that master doesn't
        compare_resp = ms.api_call(
            f"projects/{project_id}/repository/compare"
            f"?from={urllib.parse.quote(master_branch, safe='')}"
            f"&to={urllib.parse.quote(develop_branch, safe='')}"
            f"&straight=false"
        )

        if not isinstance(compare_resp, dict) or compare_resp.get('error'):
            err = compare_resp.get('details', 'Compare API failed') if isinstance(compare_resp, dict) else str(compare_resp)
            return jsonify({'success': False, 'error': err, 'has_changes': False}), 500

        commits_raw = compare_resp.get('commits', []) or []
        diffs_raw   = compare_resp.get('diffs', [])   or []

        commits = [
            {
                'id':           c.get('id', ''),
                'short_id':     c.get('short_id', (c.get('id') or '')[:8]),
                'title':        c.get('title', c.get('message', '')),
                'author_name':  c.get('author_name', 'Unknown'),
                'authored_date': c.get('authored_date', c.get('created_at', '')),
            }
            for c in commits_raw[:50]  # cap at 50 for display
        ]
        ...
        diff_files = [
            ...
            for d in diffs_raw[:200]  # cap file list
        ]
        ...
        return jsonify({
            ...
            'target':     master_branch,
            'detail':     (
                f'{len(commits)} commit(s) in {develop_branch} not yet in {master_branch}'
                if has_changes else
                f'{develop_branch} is up-to-date with {master_branch} — no MR needed'
            ),
        })

    except Exception as e:
        print(f"[ERROR] develop-master-compare for project {project_id}: {e}")
        return jsonify({'success': False, 'error': str(e), 'has_changes': False}), 500
...
@app.route('/api/projects/<int:project_id>/mr-status', methods=['GET'])
def get_mr_status(project_id):
    """Return open/merged MR details for a project, including approval state.

    Query params (optional):
        branch_num    – ticket number (default '12938')
        branch_prefix – prefix (default 'task-')
        branch_suffix – suffix (default 'java17-migration')
        mr_type       – 'feature_to_develop' (default) or 'develop_to_master'

    Returns for each matching MR:
        state           – 'opened' | 'merged' | 'closed'
        mr_iid          – MR internal ID
        mr_url          – GitLab web URL
        title           – MR title
        ...
        merged_by       – {username, name} or null
        approvals_required – number of approvals required (from project settings)
        approved        – bool (True if required approvals met)
        approved_by     – list of {username, name, avatar_url} who have approved
        pending_approvers – list of {username, name} who haven't approved yet
                           (populated from approval_rules if available)
        reviewers       – list of {username, name, avatar_url} set as reviewers
        assignees       – list of {username, name, avatar_url}
        upvotes         – upvote count (proxy for approvals on CE)
    """
    try:
        ...
        mr_type       = request.args.get('mr_type',       'feature_to_develop')

        # Build the expected MR title prefix so we can filter precisely
        # (DTM MRs raised by this tool always have title "TASK-{branch_num}: java migration")
        _expected_title_fragment = f"TASK-{branch_num}"

        # ── Collect candidate MRs ─────────────────────────────────────────
        if mr_type == 'develop_to_master':
            source_branch = _resolve_source_branch(project_id)
            # FIX (Bug 3): search by TASK-{branch_num} so only migration-related
            # develop→master MRs are returned — not every historical DTM MR
            mrs_all = []
            for target_cand in ('master', 'main'):
                for state_cand in ('opened', 'merged'):
                    resp = ms.api_call(
                        f"projects/{project_id}/merge_requests"
                        f"?source_branch={urllib.parse.quote(source_branch, safe='')}"
                        f"&target_branch={urllib.parse.quote(target_cand, safe='')}"
                        f"&state={state_cand}"
                        f"&search={urllib.parse.quote(_expected_title_fragment, safe='')}"
                        f"&per_page=10"
                    )
                    if isinstance(resp, list):
                        # Extra guard: only keep MRs whose title actually contains TASK-{branch_num}
                        mrs_all.extend(
                            m for m in resp
                            if _expected_title_fragment.lower() in (m.get('title') or '').lower()
                        )
        else:
            feature_branch = _build_feature_branch(branch_num, branch_prefix, branch_suffix)
            source_branch  = feature_branch
            # Feature branch name is unique — no extra filtering needed
            open_resp   = ms.api_call(
                f"projects/{project_id}/merge_requests"
                f"?source_branch={urllib.parse.quote(feature_branch, safe='')}&state=opened&per_page=5"
            )
            merged_resp = ms.api_call(
                f"projects/{project_id}/merge_requests"
                f"?source_branch={urllib.parse.quote(feature_branch, safe='')}&state=merged&per_page=5"
            )
            mrs_all = []
            if isinstance(open_resp,   list): mrs_all.extend(open_resp)
            if isinstance(merged_resp, list): mrs_all.extend(merged_resp)
        ...
        # Sort: opened first, then by iid descending
        unique_mrs.sort(key=lambda m: (0 if m.get('state') == 'opened' else 1, -(m.get('iid') or 0)))
        ...
        for mr in unique_mrs:
            ...
            # Strategy:
            #   1. MR-level approval_rules  → most specific, override project defaults
            #   2. Project-level approval_rules → inherited defaults (EE feature)
            #   3. .env fallback → synthesised rules from RELEASE_MANAGEMENT_USERNAMES
            #      / CODE_REVIEW_USERNAMES / REVIEWER_USERNAMES
            #
            # Each rule group: {rule_id, name, approvals_required, approved,
            #                   approved_by:[user], pending:[user], source}
            # rules_source (on the whole MR):
            #   'mr_rules' | 'project_rules' | 'env_fallback'

            def _enrich_user(raw):
                """Resolve a username string or partial dict to a full user object."""
                if isinstance(raw, str):
                    uname = raw
                    try:
                        u_resp = ms.api_call(f"users?username={urllib.parse.quote(uname, safe='')}")
                        if isinstance(u_resp, list) and u_resp:
                            return _user_obj(u_resp[0])
                    except Exception:
                        pass
                    return {'username': uname, 'name': uname, 'avatar_url': ''}
                return _user_obj(raw)

            def _build_rule_groups(rules_list, approvals_resp_approved_by):
                """Convert a GitLab approval_rules list into our structured groups."""
                # Build a set of usernames who have already approved (from the /approvals endpoint)
                approved_unames = {u['username'] for u in approvals_resp_approved_by}
                groups = []
                seen_pending_unames = set()
                for rule in rules_list:
                    rule_name      = rule.get('name') or 'Approval'
                    rule_req       = rule.get('approvals_required', 1) or 1
                    # GitLab EE: approved_by on the rule itself
                    rule_approved_by_raw = rule.get('approved_by') or []
                    rule_approved_by = []
                    for ab in rule_approved_by_raw:
                        u = ab.get('user') if isinstance(ab, dict) and 'user' in ab else ab
                        uo = _user_obj(u)
                        if uo:
                            rule_approved_by.append(uo)
                            approved_unames.add(uo['username'])

                    # Pending = eligible approvers not yet in approved set
                    eligible = rule.get('eligible_approvers') or []
                    rule_pending = []
                    for eu in eligible:
                        uo = _user_obj(eu)
                        if uo and uo['username'] not in approved_unames and uo['username'] not in seen_pending_unames:
                            rule_pending.append(uo)
                            seen_pending_unames.add(uo['username'])

                    rule_approved = len(rule_approved_by) >= rule_req

                    groups.append({
                        'rule_id':            rule.get('id'),
                        'name':               rule_name,
                        'approvals_required': rule_req,
                        'approved':           rule_approved,
                        'approved_by':        rule_approved_by,
                        'pending':            rule_pending,
                        'source':             None,   # filled by caller
                    })
                return groups

            def _env_fallback_groups(mr_type_val):
                """Build synthetic rule groups from .env when GitLab has nothing configured."""
                _rm_names = env_config.get('release_management_usernames', [])
                _cr_names = env_config.get('code_review_usernames', [])
                _rv_names = ms.REVIEWER_USERNAMES or []
                groups = []

                # Release Management group (DTM always; feature branch also)
                rm_users = [_enrich_user(u) for u in _rm_names]
                rm_users = [u for u in rm_users if u]
                if rm_users:
                    groups.append({
                        'rule_id':            None,
                ...
                # Code Review group
                cr_users = [_enrich_user(u) for u in _cr_names]
                cr_users = [u for u in cr_users if u]
                if cr_users:
                    groups.append({
                        'rule_id':            None,
                        'name':               'Code Review',
                ...
                # Reviewer group for feature→develop if we have dedicated reviewers
                if mr_type_val != 'develop_to_master' and _rv_names:
                    rv_already_in_rm = {u['username'] for u in rm_users}
                    rv_users = [_enrich_user(u) for u in _rv_names if u not in rv_already_in_rm]
                    rv_users = [u for u in rv_users if u and u['username'] not in rv_already_in_rm]
                    if rv_users:
                        groups.append({
                            'rule_id':            None,
                            'name':               'Reviewers',
            ...
            # ── Step 1: /approvals  (gives us who actually approved) ──────
            approvals_required = 0
            approved_by        = []   # flat list — derived from rule groups below
            approved           = False
            rules_source       = 'env_fallback'
            approval_rule_groups = []

            approvals_resp = ms.api_call(
                f"projects/{project_id}/merge_requests/{mr_iid}/approvals"
            )
            glob_approved_by = []
            if isinstance(approvals_resp, dict) and not approvals_resp.get('error'):
                approvals_required = approvals_resp.get('approvals_required', 0) or 0
                for ab in (approvals_resp.get('approved_by') or []):
                    u = ab.get('user') if isinstance(ab, dict) else ab
                    uo = _user_obj(u)
                    if uo:
                        glob_approved_by.append(uo)

            # ── Step 2: MR-level approval_rules ──────────────────────────
            mr_rules_resp = ms.api_call(
                f"projects/{project_id}/merge_requests/{mr_iid}/approval_rules"
            )
            mr_rules_have_eligible = (
                isinstance(mr_rules_resp, list) and
                any(r.get('eligible_approvers') for r in mr_rules_resp)
            )

            if mr_rules_have_eligible:
                approval_rule_groups = _build_rule_groups(mr_rules_resp, glob_approved_by)
                for g in approval_rule_groups:
                    g['source'] = 'mr_rules'
                    # Backfill approved_by from global list if rule doesn't have per-rule data
                    if not g['approved_by'] and glob_approved_by:
                        g['approved_by'] = [u for u in glob_approved_by
                                            if any(u['username'] == p['username'] for p in g['pending']
                                                   )] or []
                rules_source = 'mr_rules'

            else:
                # ── Step 3: Project-level approval_rules ─────────────────
                proj_rules_resp = ms.api_call(
                    f"projects/{project_id}/approval_rules"
                )
                proj_rules_have_eligible = (
                    isinstance(proj_rules_resp, list) and
                    any(r.get('eligible_approvers') for r in proj_rules_resp)
                )

                if proj_rules_have_eligible:
                    approval_rule_groups = _build_rule_groups(proj_rules_resp, glob_approved_by)
                    for g in approval_rule_groups:
                        g['source'] = 'project_rules'
                        # Since project rules don't have per-rule approved_by,
                        # assign global approved_by to the first rule that matches
                        if not g['approved_by'] and glob_approved_by:
                            g['approved_by'] = glob_approved_by[:]
                            g['approved']    = len(g['approved_by']) >= g['approvals_required']
                            # Remove from pending those who approved
                            approved_unames_set = {u['username'] for u in g['approved_by']}
                            g['pending'] = [p for p in g['pending'] if p['username'] not in approved_unames_set]
                    rules_source = 'project_rules'

                else:
                    # ── Step 4: .env fallback ─────────────────────────────
                    approval_rule_groups = _env_fallback_groups(mr_type)
                    # If there were global approvals, mark those people as approved
                    if glob_approved_by:
                        approved_unames_set = {u['username'] for u in glob_approved_by}
                        for g in approval_rule_groups:
                            approved_in_rule = [u for u in g['pending'] if u['username'] in approved_unames_set]
                            if approved_in_rule:
                                g['approved_by'] = approved_in_rule
                                g['pending']     = [u for u in g['pending'] if u['username'] not in approved_unames_set]
                                g['approved']    = len(g['approved_by']) >= g['approvals_required']
                    rules_source = 'env_fallback'

            # ── Derive flat approved / approved_by / pending from groups ──
            all_approved_by_unames = set()
            flat_approved_by = []
            flat_pending     = []
            all_rules_approved = bool(approval_rule_groups)   # starts True; any failed rule → False

            for g in approval_rule_groups:
                for u in g['approved_by']:
                    if u['username'] not in all_approved_by_unames:
                        all_approved_by_unames.add(u['username'])
                        flat_approved_by.append(u)
                for u in g['pending']:
                    flat_pending.append(u)
                if not g['approved']:
                    all_rules_approved = False

            # For merged MRs we still want to show who approved before it was merged
            if mr_state == 'merged' and glob_approved_by and not flat_approved_by:
                flat_approved_by = glob_approved_by

            approved_by        = flat_approved_by
            pending_approvers  = flat_pending
            approved           = all_rules_approved and len(flat_approved_by) > 0

                        # Reviewers and assignees from the MR object itself
            reviewers  = [_user_obj(r) for r in (mr.get('reviewers')  or []) if _user_obj(r)]
            assignees  = [_user_obj(a) for a in (mr.get('assignees')  or []) if _user_obj(a)]
            # Also try single reviewer/assignee fields
            if not reviewers and mr.get('reviewer'):
                u = _user_obj(mr.get('reviewer'))
                if u: reviewers = [u]
            if not assignees and mr.get('assignee'):
                u = _user_obj(mr.get('assignee'))
                if u: assignees = [u]

            merged_by = None
            if mr_state == 'merged' and mr.get('merged_by'):
                merged_by = _user_obj(mr.get('merged_by'))
            ...
            result_mrs.append({
                ...
                'title':               mr.get('title', ''),
                'source_branch':       mr.get('source_branch', source_branch),
                'target_branch':       mr.get('target_branch', ''),
                'author':              _user_obj(mr.get('author')),
                ...
                # Structured per-rule approval groups
                'approval_rule_groups': approval_rule_groups,
                # Where did the approver list come from?
                # 'mr_rules' | 'project_rules' | 'env_fallback'
                'rules_source':        rules_source,
                # Show reviewers only when GitLab has no approval rules configured
                'show_reviewers':      rules_source == 'env_fallback',
                'reviewers':           reviewers,
                'assignees':           assignees,
        ...
        overall_approved = any(m['approved'] for m in result_mrs if m['state'] == 'opened')
        overall_merged   = any(m['state'] == 'merged'           for m in result_mrs)

        # Count configuration quality across MRs
        _open_mrs = [m for m in result_mrs if m['state'] == 'opened']
        _rules_breakdown = {
            'mr_rules':      sum(1 for m in _open_mrs if m['rules_source'] == 'mr_rules'),
            'project_rules': sum(1 for m in _open_mrs if m['rules_source'] == 'project_rules'),
            'env_fallback':  sum(1 for m in _open_mrs if m['rules_source'] == 'env_fallback'),
        }

        return jsonify({
            'success':       True,
            'mrs':           result_mrs,
            'total':         len(result_mrs),
            'mr_type':       mr_type,
            'source_branch': source_branch,
            'summary': {
                'has_open':       any(m['state'] == 'opened' for m in result_mrs),
                'has_merged':     overall_merged,
                'all_approved':   overall_approved,
                'rules_breakdown': _rules_breakdown,
            }
        })

    except Exception as e:
        print(f"[ERROR] get_mr_status for project {project_id}: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500
...
def get_users():
    """Return Release Management and Code Review user groups configured in .env.
    Optionally enriches usernames with display names from GitLab API.

    .env keys:
      RELEASE_MANAGEMENT_USERNAMES = alice,bob
      CODE_REVIEW_USERNAMES        = charlie,dave,eve

    Returns:
      { success: true, release_management: [{username, name, id}...], code_review: [...] }
    """
    try:
        release_usernames     = list(env_config.get('release_management_usernames', []))
        code_review_usernames = list(env_config.get('code_review_usernames', []))

        def _enrich(usernames):
            enriched = []
            for uname in usernames:
                try:
                    resp = ms.api_call(f"users?username={urllib.parse.quote(uname, safe='')}")
                    if isinstance(resp, list) and resp:
                        ...
                        enriched.append({'username': uname, 'name': uname, 'id': None})
                except Exception:
                    enriched.append({'username': uname, 'name': uname, 'id': None})
            return enriched
        ...
        return jsonify({
            'success':            True,
            'release_management': _enrich(release_usernames),
            'code_review':        _enrich(code_review_usernames),
        })
    except Exception as e:
        print(f"[ERROR] /api/users: {e}")
        return jsonify({'success': False, 'error': str(e)}), 500


@app.route('/api/approval-rules', methods=['GET'])
def get_approval_rules():
    """Fetch project-level approval rules from every configured GitLab project
    and aggregate the eligible approvers, grouped by rule type.

    Rule grouping heuristic (case-insensitive rule name matching):
      - Rule name contains 'release' or 'management'  → release_management
      - Rule name contains 'review' or 'code'          → code_review
      - All others                                     → other

    Falls back to .env usernames for groups that GitLab returns no rules for,
    so the screen always shows useful defaults even if GitLab CE lacks EE rules.

    Returns:
      {
        success: true,
        release_management: [{username, name, id, avatar_url}...],
        code_review:        [{username, name, id, avatar_url}...],
        other:              [{username, name, id, avatar_url}...],
        projects_scanned:   int,
        projects_with_rules: int,
        source: 'gitlab_rules' | 'env_fallback' | 'mixed'
      }
    """
    import concurrent.futures
    ...
    def _classify_rule(rule_name: str):
        n = rule_name.lower()
        if any(k in n for k in ('release', 'management')):
            return 'release_management'
        if any(k in n for k in ('review', 'code')):
            return 'code_review'
        return 'other'

    def _fetch_rules_for_project(pid):
        try:
            resp = ms.api_call(f"projects/{pid}/approval_rules")
            if isinstance(resp, list):
                return resp
        except Exception as exc:
            print(f"[APPROVAL-RULES] project {pid} error: {exc}")
        return []

    # Optional: scope to a single project (used by frontend for per-project fingerprinting)
    _single_pid = request.args.get('project_id')
    if _single_pid:
        try:
            _single_pid = int(_single_pid)
        except (ValueError, TypeError):
            _single_pid = None

    # Fetch rules from all projects (or just the requested one) in parallel
    pid_list = [_single_pid] if _single_pid else list(PROJECT_NAMES.keys())
    projects_with_rules = 0
    all_rules_by_group = {'release_management': {}, 'code_review': {}, 'other': {}}

    if pid_list:
        with concurrent.futures.ThreadPoolExecutor(max_workers=min(10, len(pid_list))) as ex:
            results = list(ex.map(_fetch_rules_for_project, pid_list))

        for rules in results:
            if rules:
                projects_with_rules += 1
                for rule in rules:
                    group = _classify_rule(rule.get('name', ''))
                    for eu in (rule.get('eligible_approvers') or []):
                        uo = _user_obj(eu)
                        if uo and uo['username']:
                            # Deduplicate by username; last write wins (same user across projects)
                            all_rules_by_group[group][uo['username']] = uo

    # Determine source and apply .env fallback for empty groups
    source = 'gitlab_rules' if projects_with_rules > 0 else 'env_fallback'

    def _env_fallback_group(key):
        """Return enriched .env users for a group key."""
        env_key = 'release_management_usernames' if key == 'release_management' else 'code_review_usernames'
        usernames = list(env_config.get(env_key, []))
        enriched = {}
        for uname in usernames:
            try:
                resp = ms.api_call(f"users?username={urllib.parse.quote(uname, safe='')}")
                if isinstance(resp, list) and resp:
                    ...
                    enriched[uname] = {'username': uname, 'name': uname, 'id': None, 'avatar_url': ''}
            except Exception:
                enriched[uname] = {'username': uname, 'name': uname, 'id': None, 'avatar_url': ''}
        return enriched

    # If GitLab returned rules but a specific group is empty, fall back to .env for that group
    for group_key in ('release_management', 'code_review'):
        if not all_rules_by_group[group_key]:
            fallback = _env_fallback_group(group_key)
            if fallback:
                all_rules_by_group[group_key] = fallback
                if source == 'gitlab_rules':
                    source = 'mixed'

    print(f"[APPROVAL-RULES] scanned={len(pid_list)} with_rules={projects_with_rules} "
          f"rm={len(all_rules_by_group['release_management'])} "
          f"cr={len(all_rules_by_group['code_review'])} "
          f"other={len(all_rules_by_group['other'])} source={source}")

    return jsonify({
        'success':             True,
        'release_management':  list(all_rules_by_group['release_management'].values()),
        'code_review':         list(all_rules_by_group['code_review'].values()),
        'other':               list(all_rules_by_group['other'].values()),
        'projects_scanned':    len(pid_list),
        'projects_with_rules': projects_with_rules,
        'source':              source,
    })
...
def reload_config():
    ...
    try:
        ...
        # Also refresh extended config (includes presto_project_ids and approver groups)
        ...
        env_config['release_management_usernames'] = _new_ext['release_management_usernames']
        env_config['code_review_usernames']        = _new_ext['code_review_usernames']
